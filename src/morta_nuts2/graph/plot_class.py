...
class RegionalCurvePlotter:
    ...
    def _render_group(self, region_indices, t_index, year_to_plot, group_label):
        """
        Internal — renders one subplot grid for a subset of regions.

        Parameters
        ----------
        region_indices : list[int] — indices into self.regions for this group
        t_index        : int       — time index in the curves arrays
        year_to_plot   : int       — displayed in the figure title
        group_label    : str       — "1/2" or "2/2", appended to the suptitle
        ...
        # fig.suptitle(
        #     f"{self.title_prefix} — Year {year_to_plot}  [{group_label}]",
    ...
    def plot(self, year_to_plot):
        """
        Render one figure per group of n_cols regions (1 row x n_cols columns).
        With n_cols=3 (default) and 22 regions this produces 8 figures of 3 panels.
        ...
        self._fig  = self._figs[0]   # save() iterates self._figs
        plt.show()

    def save(self, path, dpi=150, **kwargs):
        """
        Save both subplot figures produced by plot().

        The path is used as a base: "_part1" and "_part2" are inserted before
        the file extension.  Example: "output/curves_2040.png" produces
        "output/curves_2040_part1.png" and "output/curves_2040_part2.png".

        Parameters
        ----------
        path : str | Path — base destination file path
        dpi  : int        — resolution in dots per inch (default: 150)
        **kwargs          — forwarded to matplotlib savefig()
        """
...
class MapPlotter:
    ...
    def save(self, path, dpi=150, **kwargs):
        ...
        Parameters
        ----------
        path : str | Path — destination file path (e.g. "output/map_2030.png")
    ...
    def plot_compare_models(
        ...
        """
        Display the errors (or any indicator) of two models side by side on a map,
        with a shared diverging colour scale so differences are immediately visible.

        Parameters
        ----------
        other_data    : ndarray — same shape as self.data (horizon, nb_regions) or
                        (nb_regions,) when static=True.
                        Represents the second model's values.
        model_labels  : tuple of two str — titles for the left and right panels
                        (default: ("Model A", "Model B"))
        static        : bool — if True, both self and other_data are treated as
                        time-independent (no year lookup). (default: False)
        year          : int | None — projection year to display when static=False.
                        If None, uses the first year in tv_future.
        cmap_diverging: str — colormap applied to *both* panels so the same colour
                        always means the same magnitude. A diverging map (e.g.
                        "RdYlGn_r", "coolwarm", "bwr") works best for errors.
                        (default: "RdYlGn_r")

        Notes
        -----
        * A shared normalisation range is computed across both models so colours
          are directly comparable between the two panels.
        * The method stores the figure in self._fig, so you can call save() after.
        ...
        title_year = "" if static else f" — {year}"
        # plt.suptitle(
        #     f"{self.indicator_name} : {model_labels[0]} vs {model_labels[1]}"
        #     f"{title_year} — {self.country_code}",
...
#     year=2040,                   # omit or set static=True for static data
#     cmap_diverging="RdYlGn_r",   # diverging cmap highlights differences
# )
